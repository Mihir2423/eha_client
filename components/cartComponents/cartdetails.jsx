  ...
  return (
    <div className="md:flex-col mx-2 md:p-2 md:ml-12 shadow-xl bg-white md:w-[900px]">
      <div>
        <Typography
          component="h1"
          className="text-neutral-700 text-2xl font-medium border-b-2 mb-4 pb-2 flex md:px-4 px-3 py-4"
        >
          Order Summary{" "}
          <Typography className="text-base m-2">
            ({items.length} items)
          </Typography>
        </Typography>
      </div>
      {items.map((item) => (
        <Card
          className={`${nova_thai.className} shadow-none rounded-none md:flex text-neutral-700 border-b-2 md:w-[880px]`}
          key={item.id}
        >
          <center>
            <Image
          ...
          {isMobile ? (
            <div className="flex md:align-center md:p-2 md:justify-end md:w-[30%] p-4 justify-between">
              ...
              <div className="flex items-center bg-red-200 rounded-md border-1">
                ...
                <Typography className="p-3 font-semibold border-2 bg-white">
                  {item?.quantity}
                </Typography>
                <IconButton
                  aria-label="plus"
          ...
          {!isMobile ? (
            <div className="flex-col flex-end  p-2 items-end justify-end">
              <div className="bg-red-200 flex rounded-md border-1">
                ...
                <div className="p-3 font-semibold border-2 bg-white">
                  {item?.quantity}
                </div>
                <IconButton
                  aria-label="negative"
              ...
              <div
                className="flex items-center ml-auto text-gray-400 mt-1"
                onClick={() => handleRemoveItem(item.id)}
              >
